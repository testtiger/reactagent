...
export default class CreateOrderPage extends React.Component {
  ...
  populateShippingMethods(){
    ...
    if(typeof self.state.items==='undefined' && self.state.items.length<0){
     ...
     if (!self.state.shippingAddress && self.isShippingAddressValid(self.state.shippingAddress)===false){
       alert("shippingAddress cannot be empty");
    }
    ...
    let localState = {...self.state};
     ...
     makePostCall(uri, headers, requestObject).then(function (endPointResponse) {
       ...
       if (endPointResponse.items) {
         ...
         self.setState({selectedShippingMethod: endPointResponse.items[0].getId()})
  ...
  render() {
    return <div className="container">
        ...
        <button className="" onClick={this.populateShippingMethods.bind(this)}>
          LoadShippingMethods
        </button>
        if(this.state.availableShippingMethods){
        <ShippingMethods shippingMethodsList={this.state.shippingMethods} selectedMethod={this.selectedShippingMethod.bind(this)} />}
