...
export default class CreateOrderPage extends React.Component {
                 constructor(props) {
                   ...
                   this.state = { profileId: this.props.match.params.profileId, items: [], orderId: null, siteId: "", plg: "", shippingAddress: "", payment: "" };
                 }
                 ...
                 getInCompleteCartItems(profileId) {
                   ...
                   makeGetCall(uri, headers, queryParams).then(
                     endPointResponse => {
                       ...
                       if (endPointResponse.shoppingCart) {
                         endPointResponse.shoppingCart.items.map(
                           function(ele) {
                             let item = {
                               ...incompletecartItemProps
                             };
                             item.resp = ele;
                             item.remove = self.removeItemFromCart.bind(
                               self
                             );
                         ...
                         localState.orderId =
                           endPointResponse.id;
                         localState.siteId =
                           endPointResponse.siteId;
                         localState.plg =
                           endPointResponse.priceListGroup;
                     ...
                     endPointResponse => {
                       ...
                       if (endPointResponse.items) {
                         ...
                         let item = {
                           ...
                           remove: self.removeItemFromCart.bind(
                             self
                           )
                         };
                         ...
                         if (
                           ...
                           self.makeCreateOrderCall(localState);
                         }
                       }
                       if (endPointResponse.errorCode) {
                         alert(endPointResponse.message);
                 ...
                 makeCreateOrderCall(response) {
                   if (this.state.items.length === 0 && this.state.orderId === null) {
                     let shoppingCartItems = this.makeShoppingCartWithItems(response);

                     let requestObject = { profileId: this.state.profileId, shoppingCart: { items: shoppingCartItems }, combineLineItems: "yes", op: "createOrder", requestChannel: "agent", payments: [] };
                     ...
                     makePostCall(uri, headers, requestObject).then(
                 ...
                 makePriceOrderCall(response) {
                   ...
                   if (self.state.items.length > 0 && self.state.orderId !== null) {
                     let shoppingCartItems = self.makeShoppingCartWithItems(localState);

                     let requestObject = { id: self.state.orderId, shoppingCart: { items: shoppingCartItems }, combineLineItems: "yes", op: "priceOrder", payments: [] };
                 ...
                 makeShoppingCartWithItems(local) {
                   let extraProps = { expanded: false, stockStatus: true, stockState: "", commerceItemQuantity: 1, detailedItemPriceInfo: [], detailedRecurringChargeInfo: [], externalData: [], addOnItem: false, invalid: false, priceListGroupId: "defaultPriceGroup" };
                   let localState = local ? local : this.state;
                   if (localState.items.length > 0) {
                     return localState.items.map(function(item) {
                       return { productId: item.productId(), quantity: item.quantity(), repositoryId: "", catRefId: item.skuId(), displayName: item.displayName() };
                     });
                   } else {
                     return;
                   }
                 ...
                 render() {
                   return <div className="container">
                       ...
                       <hr />
                       <div className="row">
                         <AddOptions addBySku={this.addBySku.bind(this)} addByName={this.addByProductName.bind(this)} />
                       </div>
                       <div className="row">
                       ...
                       <button
                         onClick={this.onClickPlaceOrder.bind(
                           this
                         )}
                       >
                         Placeorder
