...
export default class CreateOrderPage extends React.Component {
  constructor(props) {
    ...
    var 
    this.inCompleteCartItems = [];
    function extractItems(endPointResponse) {
        this.inCompleteCartItems = endPointResponse.shoppingCart.items.map(function(ele) {
    ...
    let inCompleteCartItems = this.getInCompleteCartItems(
        this.props.match.params.profileId, extractItems.bind(this)
    );
    this.state = {
      profileId: this.props.match.params.profileId,
      items: this.inCompleteCartItems
    };
  }
  ...
  getInCompleteCartItems(profileId) {
    ...
    let incompleteCartItems = [];
    makeGetCall(uri, headers, queryParams).then(endPointResponse => {
      console.log(" response  ============>", endPointResponse);
      if (endPointResponse.shoppingCart.items) {
        function extractItems() {
          var itemsArray = endPointResponse.shoppingCart.items.map(function(
            ele
          ) {
            let item = { ...incompletecartItemProps };
            item.resp = ele;
