...
export default class CreateOrderPage extends React.Component {
  ...
  makeCreateOrderCall(localState) {
    if (this.state.items.length === 0 && this.stateorderId === null) {
      let shoppingCartItems = this.makeShoppingCartWithItems(localState);
  ...
  makePriceOrderCall(localState) {
    if (this.state.items.length > 0 && this.state.orderId !== null) {
      let shoppingCartItems = this.makeShoppingCartWithItems(localState);

      let requestObject = { id: this.state.profileId, shoppingCart: { items: shoppingCartItems }, combineLineItems: "yes", op: "priceOrder", requestChannel: "agent", payments: [] };
