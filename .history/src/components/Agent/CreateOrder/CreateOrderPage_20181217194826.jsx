...
export default class CreateOrderPage extends React.Component {
  ...
  populateShippingMethods() {
    let self = this;
    if (
      !(typeof self.state.items === "undefined" && self.state.items.length > 0)
    ) {
    ...
    if (
      !(
        ...
        self.isShippingAddressValid(self.state.shippingAddress)
      )
    ) {
      throw TypeError("shopping shoulnot be empty/undefined");
    }
    var requestObject = makeShoppingCartWithItems();
  }
  render() {
    return (
